...
fn parsed_opcode() {
    ...
    for mut op in data.clone() {
        ...
        let op_r = ParsedOpcode::from_bytes_unchecked(op_bytes.as_slice());
        let op_p = ParsedOpcode::from(op_p);

        assert_eq!(op, Opcode::from(op_r));
        assert_eq!(op, Opcode::from(op_p));
        ...
        while op_bytes.len() > Opcode::BYTES_SIZE {
            ...
            let op_r = ParsedOpcode::from_bytes_unchecked(op_bytes.as_slice());
            let op_s = ParsedOpcode::from_bytes(op_bytes.as_slice())
                .expect("Failed to safely generate op from bytes!");

            assert_eq!(op, Opcode::from(op_r));
            assert_eq!(op, Opcode::from(op_s));
        }
